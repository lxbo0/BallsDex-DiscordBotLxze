...
RUN --mount=type=cache,id=cache1-pip,target=/root/.cache/pip pip install -r requirements.txt
    uv venv $VIRTUAL_ENV && \
    uv sync --locked --no-install-project --no-editable --active
COPY --parents admin_panel ballsdex LICENSE README.md /code/
RUN --mount=type=cache,id=cache2-pip,target=/root/.cache/pip pip install -r requirements.txt
...
RUN --mount=type=cache,id=cache3-pip,target=/root/.cache/pip pip install -r requirements.txt
